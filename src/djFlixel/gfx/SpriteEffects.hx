 ...
 * - Effects can be stacked on top of eachother
 * - Uses Per pixel operations done in software, so not ideal for large areas
 * 
 * Note :
 *  - This isn't nice code and the whole class is 
 *    almost like a quick draft that needs to be refactored , but it works.
 *  - Some effects are really slow on some targets, initially written for FLASH
 *    e.g. html5 is a crawl.
 * 
 * Effect List :
 *   - dissolve
 *   - wave
...
class SpriteEffects extends FlxSprite
{
	...
	/**
	 * @param	im Bitmapdata or Asset
	 * @param	sheet { .tw .th .frame } Set this if you are loading a tilesheet
	 */
	public function new(im:FlxGraphicSource, ?sheet:Dynamic) 
	{
		super();
		
		if (sheet != null)
		{
			_im = D.bmu.getBitmapSquare(FlxAssets.resolveBitmapData(im), Std.int(sheet.tw * sheet.frame), 0, sheet.tw, sheet.th);
			flag_unique_src = true;
		}
		else
		{
			_im = FlxAssets.resolveBitmapData(im);
	 ...
	 * @param	params check the code. Note you can set an .id if you want to use removeEffect later
	...
	override public function update(elapsed:Float):Void 
	{
		...
		if ((timer += elapsed) > FREQ && (stack.length > 0))
		{
			timer = 0;
			pixels.lock();
