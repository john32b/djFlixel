...
class State_MenuDemo1 extends FlxState
{
	...
	var description = [
			...
			"Dynamically Creating dialogs and popups for links with a confirmation flag set\n" +
			"---\n" +
			"Keys: #WASD J/K# | #Arrows Z/X# ",
	...
	override public function create():Void
	{
		...
		var P = DataTool.copyFieldsC(FLS.JSON.STATE_MENU_DEMO1); 
		// DataTool.copyFieldsC Translates color strings to actual Ints in the object
		
		// -- A Gradient bg for style
		var bg = new FlxSprite(0, 0, 
			FlxGradient.createGradientBitmapData(FlxG.width, FlxG.height, cast P.colorGrad, 16));
		add(bg);
		...
		var p:PageData = menu.newPage("main", {title:"Functionality Demo", desc:description[0]});
		...
		// Slider with floats
		p.add("Slider 2", {type:"slider", sid:"it_4", pool:[0, 1], inc:0.02, desc:"Selects a float between a range"});
		...
		p.custom.styleMenu = {
			el_scroll_time:0.06 // Make the scrolling faster
		};
	...
	// Menu callbacks
	function menuCallbacks(s:String, data:String, item:MItemData)
	{
		...
		if (s == "pageOn")
		{
			...
			{
				infoBox.open(menu.currentPage.description);
			}
		}// --
		...
		{
			desc.text = (item.description != null)?item.description:"";
		}
	...
	override public function update(elapsed:Float):Void 
	{
		...
		{
			if (menu.visible) menu.close(true); else menu.open();
		}
